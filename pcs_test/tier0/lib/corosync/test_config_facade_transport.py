...
class IsEncryptionEnabled(TestCase):
    @staticmethod
    def fixture_config_knet(knet_cipher, knet_hash):
        return dedent(
            f"""\
            totem {{
                version: 2
                ...
                transport: knet
                crypto_cipher: {knet_cipher}
                crypto_hash: {knet_hash}
            }}
            """
        )

    def check_knet_encryption_enabled(self, knet_cipher, knet_hash):
        return _get_facade(
            self.fixture_config_knet(knet_cipher, knet_hash)
        ).is_encryption_enabled()

    def test_knet_cipher_none(self):
        self.assertFalse(self.check_knet_encryption_enabled("none", "sha256"))

    def test_knet_hash_none(self):
        self.assertFalse(self.check_knet_encryption_enabled("aes256", "none"))

    def test_knet_both_none(self):
        self.assertFalse(self.check_knet_encryption_enabled("none", "none"))

    def test_knet_encryption_enabled(self):
        self.assertTrue(self.check_knet_encryption_enabled("aes256", "sha256"))

    def test_knet_defaults(self):
        self.assertFalse(
            _get_facade(
                dedent(
                    """\
